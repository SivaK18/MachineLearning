...
def train_weights(train, l_rate, n_epoch):
	...
	for epoch in range(n_epoch):
		...
		plot(dataset,weights);dat.append((epoch,format(weights[0], '.3f'),format(weights[1], '.3f'),format(weights[2], '.3f'),l_rate,sum_error));  df=pd.DataFrame(dat); df.to_csv('dat.csv', mode='a', header=False, index =False)
	return weights
